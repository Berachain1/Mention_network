...
function displayBanner() {
  ...
  const banner = figlet.textSync('ANNISA', {
    font: "ANSI Shadow",
  ...
  lines.forEach((line, index) => {
    ...
    if (index < lines.length / 2) {
      console.log(chalk.redBright(padded));
    } else {
      console.log(chalk.whiteBright(padded));
    }
  });

  const info1 = '=== Telegram Channel 🚀 : AIRDROPRSIND123 ( https://t.me/AIRDROPRSIND123 ) ===';
  const info2 = '✪ MENTION NETWORK ✪';

  console.log(
    chalk.cyanBright(' '.repeat(Math.floor((width - info1.length) / 2)) + info1)
  );
  console.log(
    chalk.yellowBright(' '.repeat(Math.floor((width - info2.length) / 2)) + info2 + '\n')
  );
}
